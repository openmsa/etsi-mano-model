...
/**
 * This type represents a subscription filter related to notifications about the
 * availability of the virtualised resources quotas. At a particular nesting
 * level in the filter structure, the following applies: All attributes shall
 * match in order for the filter to match (logical \&quot;and\&quot; between
 * different filter attributes). If an attribute is an array, the attribute
 * shall match if at least one of the values in the array matches (logical
...
public class VrQuotaAvailNotificationsFilter {
	...
	/**
	 * Match VIMs that were created the quota for a consumer of the virtualised
	 * resources. This attribute shall only be supported when VNF-related Resource
	 ...
	 * resources that were allocated by the NFVO. This attribute shall only be
	...
	/**
	 * Match the \"infrastructure resource groups\" that are logical groupings of
	 * the virtualised resources assigned to a tenant within an infrastructure
	 * Domain.
	 *
