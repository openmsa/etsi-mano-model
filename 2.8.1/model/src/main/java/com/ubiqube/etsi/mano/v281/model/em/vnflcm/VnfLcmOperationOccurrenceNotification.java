...
public class VnfLcmOperationOccurrenceNotification {
	 ...
	 * operation. NOTE 1: Shall be present if the \"notificationStatus\" is set to
	 * \"RESULT\" and the operation has performed any resource modification. Shall
	 * be absent otherwise. This attribute contains information about the cumulative
	 * changes to virtualised resources that were performed so far by the VNF LCM
	 * operation occurrence and by any of the error handling procedures for that
	 * operation occurrence. NOTE 2: For a particular affected VL, there shall be as
	 * many \"AffectedVirtualLink\" entries as needed for signalling the different
	 ...
	 * \"AffectedVirtualLink\" entry signals the addition of externally visible VNF
	 ...
	 * lifecycle operation. See note 1. NOTE 1: Shall be present if the
	 * \"notificationStatus\" is set to \"RESULT\" and the operation has performed
	 * any resource modification. Shall be absent otherwise. This attribute contains
	 * information about the cumulative changes to virtualised resources that were
	 * performed so far by the VNF LCM operation occurrence and by any of the error
	 ...
	 * absent otherwise. Only information about external VL instances that have been
	 * added or modified shall be provided.
	 *
	 * @return changedExtConnectivity
	 **/
	@Schema(description = "Information about changed external connectivity, if this notification represents the result of a lifecycle operation occurrence. Shall be present if the \"notificationStatus\" is set to \"RESULT\" and the \"operation\" has made any changes to the external connectivity of the VNF instance. Shall be absent otherwise. Only information about external VL instances that have been added or modified shall be provided. ")
	@Valid
	...
	@Override
	public int hashCode() {
		return Objects.hash(id, notificationType, subscriptionId, timeStamp, notificationStatus, operationState, vnfInstanceId, operation, isAutomaticInvocation, vnfLcmOpOccId, affectedVnfcs, affectedVirtualLinks, affectedExtLinkPorts, affectedVirtualStorages, changedInfo, changedExtConnectivity, error, _links);
	}
