...
import com.ubiqube.etsi.mano.v281.service.mapping.pkg.ExternalArtifactsAccessConfig281Mapping;
...
public class VnfPackages281Sol005Controller implements VnfPackages281Sol005Api {
	...
	private final ExternalArtifactsAccessConfig281Mapping externalMapping;

	public VnfPackages281Sol005Controller(final VnfPackageFrontController frontController, final VnfPkgInfo281Mapping mapper, final ExternalArtifactsAccessConfig281Mapping externalMapping) {
	...
	public ResponseEntity<String> vnfPackagesGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return frontController.search(requestParams, x -> mapper.map(x), VnfPackages281Sol005Controller::makeLinks, VnfPkgInfo.class);
	}

	@Override
	public ResponseEntity<VnfPkgInfo> vnfPackagesPost(@Valid final CreateVnfPkgInfoRequest body) {
		return frontController.create(body.getUserDefinedData(), x -> mapper.map(x), VnfPackages281Sol005Controller::makeLinks, VnfPackages281Sol005Controller::getSelfLink);
	}
	...
	public ResponseEntity<VnfPkgInfo> vnfPackagesVnfPkgIdGet(final String vnfPkgId) {
		return frontController.findById(getSafeUUID(vnfPkgId), x -> mapper.map(x), VnfPackages281Sol005Controller::makeLinks);
	}
	...
	public ResponseEntity<VnfPkgInfo> vnfPackagesVnfPkgIdPatch(@Valid final String body, final String vnfPkgId, final String ifMatch) {
		return frontController.modify(body, getSafeUUID(vnfPkgId), ifMatch, x -> mapper.map(x), VnfPackages281Sol005Controller::makeLinks);
	}
