...
/**
 * This type represents configuration data for a logging job. It shall comply
 * with the provisions defined in table 8.6.3.6-1. NOTE: The present document
 * version does not specify the support for \&quot;log compilation and reporting
 * based on events\&quot; as specified in clause 6.6.2.2 of ETSI GS NFV-IFA 031.
 * * NOTE 1: The minimumReportingPeriod is used to throttle possible flooding of
 * reports by providing a lower limit on the gap between two log availability
 * notification to be emitted by the same logging job. NOTE 2: More than one
 * logCompilingCondition is possible to provide a combination of different
 * conditions for the compilation. This covers use cases when the compilation of
 * the log can be based on, e.g. a timer value and a size value, whichever first
 * condition is met first. For instance, compile a log every 3 600 seconds, or
 * whenever during the 3 600 seconds period the log grows in size to 1 000 000
 * bytes. NOTE 3: The value is indicative, as the actual size of the compiled
 * log files might not match the provided compileBySizeValue due to possible
 * deviations in the tracking of the size of the log data by the API producer,
 * and the size of individual logged data entries when these are collected.
 */
...
public class LoggingJobConfig {
	...
	public LoggingJobConfig startTime(final OffsetDateTime startTime) {
	...
	public LoggingJobConfig endTime(final OffsetDateTime endTime) {
	...
	public LoggingJobConfig reportingCondition(final LoggingJobConfigReportingCondition reportingCondition) {
	...
	public void setReportingCondition(final LoggingJobConfigReportingCondition reportingCondition) {
		this.reportingCondition = reportingCondition;
	}

	public LoggingJobConfig compileBySizeValue(final Integer compileBySizeValue) {
		this.compileBySizeValue = compileBySizeValue;
		return this;
	}

	/**
	 * An indicative size threshold for compiling the collected log data, in bytes.
	 * It is used when the compilation is based on the size of the collected log
	 * data. If not present, a default value as specified with the
	 * \"defaultLogCompileBySizeValue\" configuration in the
	 * \"ManoEntityConfigurableParams\" shall be used. See note 2 and note 3.
	 *
	...
	public void setCompileBySizeValue(final Integer compileBySizeValue) {
		this.compileBySizeValue = compileBySizeValue;
	}

	public LoggingJobConfig compileByTimerValue(final Integer compileByTimerValue) {
		this.compileByTimerValue = compileByTimerValue;
		return this;
	}

	/**
	 * The periodicity threshold for compiling the filtered log, in seconds. It is
	 * used when the compilation is based on a timer (e.g., every 24 hours). If not
	 * present, a default value as specified with the
	 * \"defaultLogCompileByTimerValue\" configuration in the
	 * \"ManoEntityConfigurableParams\" shall be used. See note 2.
	 *
	...
	public void setCompileByTimerValue(final Integer compileByTimerValue) {
		this.compileByTimerValue = compileByTimerValue;
	}

	public LoggingJobConfig securityConf(final LoggingJobConfigSecurityConf securityConf) {
	...
	public void setSecurityConf(final LoggingJobConfigSecurityConf securityConf) {
		this.securityConf = securityConf;
	}
	...
	public boolean equals(final java.lang.Object o) {
		...
		final LoggingJobConfig loggingJobConfig = (LoggingJobConfig) o;
