 ...
 * one or a set of OS containers and compare it to the desired state. For an
 * MCIO related to a VDU that has the attribute
 * \&quot;isNumOfInstancesClusterBased\&quot; set to FALSE the desired state is
 * specified in the respective declarative descriptor. For an MCIO related to a
 * VDU that has the attribute \&quot;isNumOfInstancesClusterBased\&quot; set to
 * TRUE, the desired state is determined by the number of CIS-nodes in the
 * cluster that fulfil the VDU requirements. It triggers actions toward the CIS
 * to align the actual to the desired state. Monitoring the actual state
 * includes monitoring the number of MCIO instances available at any specific
 * point in time. In addition, an MCIO controller maintains properties and
 * runtime information on the MCIO instances which have been created based on
 * the same VDU. The McioInfo data structure provides the runtime information on
 * the MCIOs obtained from the MCIO controller. NOTE: There are different types
 * of MCIOs. The set of VNFC instances based on the same VDU is represented by
 * one MCIO, e.g. of type Deployment. Each individual VNFC instance is
 * represented by another type of MCIO, e.g. a POD. Runtime information of the
 * set of OS containers realizing an individual VNFC instance is not part of the
 * McioInfo data structure; such runtime information is provided in the
 * ResourceHandle data structure referenced from the VnfcResourceInfo. The
 * McioInfo does not provide runtime information of a constituent VNFC instance
 * created based on a specific VDU. NOTE 1: The type of MCIO as specified in the
 * declarative descriptor of the MCIO, and that can be read from the CISM.
 * EXAMPLE: In case of MCIOs managed by Kubernetes®, the type of MCIO
 * corresponds to the “kind” property of the declarative descriptor. NOTE 2: If
 * the attribute additionalInfo is present, it may contain runtime information
 * on the actual and desired state of the MCIO(s). NOTE 3: When the container
 * infrastructure service is a Kubernetes® instance, the mcioId is the combined
 * values from the kind and name fields of the Kubernetes resource object,
 * separated by a slash. Example: \&quot;Deployment/abcd\&quot;. NOTE 4: When
 * the container infrastructure service is a Kubernetes® instance, the mcioName
 * is the name field of the resource object.
 */
...
public class McioInfo {
	...
	public McioInfo certificateContentId(final String certificateContentId) {
	...
	public boolean equals(final java.lang.Object o) {
		...
		return Objects.equals(this.mcioId, mcioInfo.mcioId) &&
				...
				Objects.equals(this.certificateContentId, mcioInfo.certificateContentId);
	}

	@Override
	public int hashCode() {
		return Objects.hash(mcioId, mcioName, mcioNamespace, vduId, cismId, mcioType, desiredInstances, availableInstances, additionalInfo, certificateContentId);
	}
