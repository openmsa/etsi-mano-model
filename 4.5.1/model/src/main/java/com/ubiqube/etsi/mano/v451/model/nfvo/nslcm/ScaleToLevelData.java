 ...
 * operation towards the appropriate VNFM. NOTE 1: Either the
 * vnfInstantiationLevelId attribute or the vnfScaleInfo attribute shall be
 * included. NOTE 2: If the referenced instantiationLevel or the scaleInfo
 * attribute contain information related to VNFCs that are not going to be
 * instantiated due to the selection of deployable modules, when passed to the
 * VNFM, the information is stored in the VNFM for later use and included in the
 * instantiatedVnfInfo, as described in clause 5.5.2.6 of ETSI GS NFV-SOL 003.
 */
@Schema(description = "This type describes the information used to scale a VNF instance to a target size. The target size is either expressed as an instantiation level of that DF as defined in the VNFD, or given as a list of scale levels, one per scaling aspect of that DF. Instantiation levels and scaling aspects are declared in the VNFD. The NFVO shall then invoke the ScaleVnfToLevel operation towards the appropriate VNFM. NOTE 1: Either the vnfInstantiationLevelId attribute or the vnfScaleInfo attribute shall be included. NOTE 2: If the referenced instantiationLevel or the scaleInfo attribute contain information related to VNFCs that are not         going to be instantiated due to the selection of deployable modules, when passed to the VNFM, the information         is stored in the VNFM for later use and included in the instantiatedVnfInfo, as described in clause 5.5.2.6 of         ETSI GS NFV-SOL 003. ")
@Validated
...
public class ScaleToLevelData implements AnyOfScaleToLevelData {
	 ...
	 * @return vnfScaleInfo
	 **/
	@Schema(description = "For each scaling aspect of the current deployment flavor, indicates the target scale level to which the VNF is to be scaled. See notes 1 and 2. ")
	@Valid
