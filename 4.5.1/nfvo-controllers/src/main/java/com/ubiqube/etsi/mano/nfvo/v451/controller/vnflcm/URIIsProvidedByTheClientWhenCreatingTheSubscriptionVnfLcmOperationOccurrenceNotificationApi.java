...
public interface URIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfLcmOperationOccurrenceNotificationApi {
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfLcmOperationOccurrenceNotificationPost(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.DEFAULT, description = "A notification about lifecycle changes triggered by a VNF LCM operation occurrence..", required = true, schema = @Schema()) @Valid @RequestBody final VnfLcmOperationOccurrenceNotification body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfLcmOperationOccurrenceNotificationApi interface so no example is generated");
		}
