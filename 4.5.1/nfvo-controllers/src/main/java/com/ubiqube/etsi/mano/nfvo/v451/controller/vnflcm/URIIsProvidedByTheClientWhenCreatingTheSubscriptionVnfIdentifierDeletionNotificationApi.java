...
import com.ubiqube.etsi.mano.em.v451.model.vnflcm.VnfIdentifierDeletionNotification;
...
public interface URIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfIdentifierDeletionNotificationApi {
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfIdentifierDeletionNotificationPost(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.DEFAULT, description = "A notification about the deletion of a VNF identifier and the related \"Individual VNF instance\" resource.", required = true, schema = @Schema()) @Valid @RequestBody final VnfIdentifierDeletionNotification body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionVnfIdentifierDeletionNotificationApi interface so no example is generated");
		}
