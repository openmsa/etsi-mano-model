...
import com.ubiqube.etsi.mano.v451.model.nfvo.vnf.PkgmSubscriptionLinks;
...
@RestController
public class VnfSubscriptions451Sol005Controller implements VnfSubscriptions451Sol005Api, SubscriptionLinkable451Nfvo {
	private final VnfSubscriptionSol005FrontController vnfSubscriptionSol03FrontController;
	private final PkgmSubscriptionRequest451Mapping mapper;

	public VnfSubscriptions451Sol005Controller(final VnfSubscriptionSol005FrontController vnfSubscriptionSol03FrontController, final PkgmSubscriptionRequest451Mapping mapper) {
	...
	public ResponseEntity<List<PkgmSubscription>> subscriptionsGet(final String filter, final String nextpageOpaqueMarker) {
		return vnfSubscriptionSol03FrontController.search(filter, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions451Sol005Controller::makeLinks);
	}
	...
	public ResponseEntity<PkgmSubscription> subscriptionsPost(final PkgmSubscriptionRequest body) {
		...
		return vnfSubscriptionSol03FrontController.create(req, VnfSubscriptions451Sol005Api.class, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions451Sol005Controller::makeLinks);
	}
	...
	public ResponseEntity<PkgmSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return vnfSubscriptionSol03FrontController.findById(subscriptionId, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions451Sol005Controller::makeLinks);
	}
	...
	public static void makeLinks(final PkgmSubscription pkgmSubscription) {
		...
		self.setHref(linkTo(methodOn(VnfSubscriptions451Sol005Api.class).subscriptionsSubscriptionIdGet(pkgmSubscription.getId())).withSelfRel().getHref());
	...
	public String makeSelfLink(final String id) {
		return linkTo(methodOn(VnfSubscriptions451Sol005Api.class).subscriptionsSubscriptionIdGet(id)).withSelfRel().getHref();
	}
