...
public interface LogNotification451Api {
	...
	Logger log = LoggerFactory.getLogger(LogNotification451Api.class);
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "204", description = "Shall be returned to indicate that the notification endpoint has been tested successfully. The response body shall be empty."),
	...
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-LogNotification", produces = { "application/json" }, method = RequestMethod.GET)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionLogNotificationGet(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.HEADER, description = "Content-Types that are acceptable for the response. Reference: IETF RFC 7231. ", required = true, schema = @Schema()) @RequestHeader(value = "Accept", required = true) final String accept, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionLogNotificationApi interface so no example is generated");
		}
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	}

	@Operation(summary = "", description = "The POST method delivers a notification regarding a log management event from the API producer to the API consumer. The API consumer shall have previously created an \"Individual subscription\" resource with a matching filter. See clause 8.5.9.3.1.", tags = {})
	@ApiResponses(value = {
			@ApiResponse(responseCode = "204", description = "Shall be returned when the notification has been delivered successfully."),
			...
			@ApiResponse(responseCode = "504", description = "504 GATEWAY TIMEOUT If the API producer encounters a timeout while waiting for a response from an upstream server (i.e. a server that the API producer communicates with when fulfilling a request), it should respond with this response code. ", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetails.class))) })
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-LogNotification", produces = { "application/json" }, consumes = { "application/json" }, method = RequestMethod.POST)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionLogNotificationPost(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.DEFAULT, description = "Notification about the availability of a log report.", required = true, schema = @Schema()) @Valid @RequestBody final LogReportAvailableNotification body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionLogNotificationApi interface so no example is generated");
		}
