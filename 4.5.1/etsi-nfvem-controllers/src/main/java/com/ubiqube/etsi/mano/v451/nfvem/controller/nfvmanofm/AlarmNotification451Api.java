...
@Validated
public interface AlarmNotification451Api {

	Logger log = LoggerFactory.getLogger(AlarmNotification451Api.class);
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmNotificationGet(
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmNotificationPost(
			@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version,
			...
			@Parameter(in = ParameterIn.DEFAULT, description = "Information of a NFV-MANO functional entity alarm.", required = true, schema = @Schema()) @Valid @RequestBody final AlarmNotification body,
			@Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmNotificationApi interface so no example is generated");
