...
public interface ConfigurationAndInformationNotification451Api {
	...
	@Operation(summary = "", description = "TThe GET method allows the API producer to test the notification endpoint that  is provided by the API consumer, e.g., during the subscription process. See clause 5.5.6.3.2. ", tags = {})
	@ApiResponses(value = {
	...
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-ConfigurationAndInformationNotification", produces = { "application/json" }, method = RequestMethod.GET)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionConfigurationAndInformationNotificationGet(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.HEADER, description = "Content-Types that are acceptable for the response. Reference: IETF RFC 7231. ", required = true, schema = @Schema()) @RequestHeader(value = "Accept", required = true) final String accept, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionConfigurationAndInformationNotificationApi interface so no example is generated");
		}
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	}

	@Operation(summary = "", description = "The POST method delivers a notification from the API producer to an API consumer. The API consumer shall have previously created an \"Individual subscription\" resource with a matching filter. See clause 5.5.6.3.1. ", tags = {})
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "504", description = "504 GATEWAY TIMEOUT If the API producer encounters a timeout while waiting for a response from an upstream server (i.e. a server that the API producer communicates with when fulfilling a request), it should respond with this response code. ", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetails.class))) })
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-ConfigurationAndInformationNotification", produces = { "application/json" }, consumes = { "application/json" }, method = RequestMethod.POST)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionConfigurationAndInformationNotificationPost(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.HEADER, description = "Content-Types that are acceptable for the response. Reference: IETF RFC 7231. ", required = true, schema = @Schema()) @RequestHeader(value = "Accept", required = true) final String accept, @Parameter(in = ParameterIn.DEFAULT, description = "A notification about configuration and information data changes of the  NFV-MANO functional entity, or state changes of the NFV-MANO functional  entity and its managed objects.", required = true, schema = @Schema()) @Valid @RequestBody final Object body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionConfigurationAndInformationNotificationApi interface so no example is generated");
		}
