...
@Validated
public interface AlarmListRebuiltNotification451Api {

	Logger log = LoggerFactory.getLogger(AlarmListRebuiltNotification451Api.class);
	...
	@Operation(summary = "", description = "The GET method allows the server to test the notification endpoint that is provided by the client, e.g. during subscription. See clause 7.5.7.3.2. ", tags = {})
	@ApiResponses(value = {
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmListRebuiltNotificationGet(
	...
	@Operation(summary = "", description = "The POST method notifies an NFV-MANO functional entity alarm or that the alarm list has been rebuilt. The API consumer shall have previously created an \"Individual subscription\" resource with a matching filter. See clause 7.5.7.3.1. ", tags = {})
	@ApiResponses(value = {
	...
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmListRebuiltNotificationPost(
			@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version,
			...
			@Parameter(in = ParameterIn.DEFAULT, description = "Information that the alarm list has been rebuilt by the NFV-MANO functional entity.", required = true, schema = @Schema()) @Valid @RequestBody final AlarmListRebuiltNotification body,
			@Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmListRebuiltNotificationApi interface so no example is generated");
