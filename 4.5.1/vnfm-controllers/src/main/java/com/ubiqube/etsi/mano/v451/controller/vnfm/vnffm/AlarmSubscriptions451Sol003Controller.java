 ...
 */
package com.ubiqube.etsi.mano.v451.controller.vnfm.vnffm;
...
@RestController
public class AlarmSubscriptions451Sol003Controller implements AlarmSubscriptions451Sol003Api, SubscriptionLinkable451Vnfm {
	...
	public AlarmSubscriptions451Sol003Controller(final FaultMngtSubscriptionsFrontController faultMngtSubscriptionsFrontController, final FmSubscription451Mapping mapper) {
	...
	public ResponseEntity<List<FmSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return faultMngtSubscriptionsFrontController.search(requestParams, x -> mapper.map(x, FmNotificationsFilter.class), AlarmSubscriptions451Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<FmSubscription> subscriptionsPost(@Valid final FmSubscriptionRequest body) {
		...
		return faultMngtSubscriptionsFrontController.create(req, x -> mapper.map(x, FmNotificationsFilter.class), AlarmSubscriptions451Sol003Api.class, AlarmSubscriptions451Sol003Controller::makeLinks, AlarmSubscriptions451Sol003Controller::makeSelf);
	}
	...
	public ResponseEntity<FmSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return faultMngtSubscriptionsFrontController.findById(subscriptionId, x -> mapper.map(x, FmNotificationsFilter.class), AlarmSubscriptions451Sol003Controller::makeLinks);
	}
	...
	private static void makeLinks(final FmSubscription subscription) {
		...
		link.setHref(linkTo(methodOn(AlarmSubscriptions451Sol003Api.class).subscriptionsSubscriptionIdGet(subscription.getId())).withSelfRel().getHref());
		links.setSelf(link);
		subscription.setLinks(links);
	}

	private static String makeSelf(final FmSubscription subscription) {
		return linkTo(methodOn(AlarmSubscriptions451Sol003Api.class).subscriptionsSubscriptionIdGet(subscription.getId())).withSelfRel().getHref();
	}

	@Override
	public String makeSelfLink(final String id) {
		return linkTo(methodOn(AlarmSubscriptions451Sol003Api.class).subscriptionsSubscriptionIdGet(id)).withSelfRel().getHref();
	}
