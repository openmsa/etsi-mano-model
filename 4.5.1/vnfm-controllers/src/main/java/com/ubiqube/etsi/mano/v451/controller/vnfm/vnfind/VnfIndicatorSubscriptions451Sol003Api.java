...
@Validated
public interface VnfIndicatorSubscriptions451Sol003Api {

	Logger log = LoggerFactory.getLogger(VnfIndicatorSubscriptions451Sol003Api.class);
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "422", description = "422 Unprocessable Content  Shall be returned when a subscription with the same callback URI and the same filter already exists and the policy of the VNFM is to not create redundant subscriptions. The HTTP response shall include a \"Location\" HTTP header that contains the resource URI of the existing \"Individual subscription\" resource. The response body shall be empty ", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetails.class))),
