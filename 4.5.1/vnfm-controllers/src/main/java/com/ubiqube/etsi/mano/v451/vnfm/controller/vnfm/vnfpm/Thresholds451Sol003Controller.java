...
@RestController
public class Thresholds451Sol003Controller implements Thresholds451Sol003Api {
	private final VnfmThresholdFrontController vnfmThresholdFrontController;
	private final Threshold451Mapping mapper;

	public Thresholds451Sol003Controller(final VnfmThresholdFrontController vnfmThresholdFrontController, final Threshold451Mapping mapper) {
	...
	public ResponseEntity<Threshold> thresholdsPost(@Valid final CreateThresholdRequest createThresholdRequest) {
		...
		return vnfmThresholdFrontController.thresholdsCreate(req, x -> mapper.map(x), Thresholds451Sol003Controller::makeLinks, Thresholds451Sol003Controller::getSelfLink);
	}
	...
	public ResponseEntity<Threshold> thresholdsThresholdIdGet(final String thresholdId) {
		return vnfmThresholdFrontController.findById(thresholdId, x -> mapper.map(x), Thresholds451Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<String> thresholdsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return vnfmThresholdFrontController.search(requestParams, nextpageOpaqueMarker, x -> mapper.map(x), Thresholds451Sol003Controller::makeLinks, Threshold.class);
	}
	...
	private static void makeLinks(final Threshold x) {
		...
		Link link = new Link();
		link.setHref(linkTo(methodOn(Thresholds451Sol003Api.class).thresholdsThresholdIdGet(x.getId())).withSelfRel().getHref());
		links.setSelf(link);
	...
	private static String getSelfLink(final Threshold threshold) {
		return linkTo(methodOn(Thresholds451Sol003Api.class).thresholdsThresholdIdGet(threshold.getId().toString())).withSelfRel().getHref();
	}
}
