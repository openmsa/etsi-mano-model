...
@RestController
public class Alarms451Sol003Controller implements Alarms451Sol003Api {
	private final AlarmFrontController alarmFrontController;
	private final Alarm451Mapping mapper;

	public Alarms451Sol003Controller(final AlarmFrontController alarmFrontController, final Alarm451Mapping mapper) {
	...
	public ResponseEntity<Alarm> alarmsAlarmIdGet(final String alarmId) {
		return alarmFrontController.findById(alarmId, x -> mapper.map(x), Alarms451Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<String> alarmsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return alarmFrontController.search(requestParams, x -> mapper.map(x), Alarms451Sol003Controller::makeLinks, Alarm.class);
	}

	private static void makeLinks(final Alarm alarm) {
		final AlarmLinks links = new AlarmLinks();
		Link link = new Link();
		link.setHref(linkTo(methodOn(Alarms451Sol003Api.class).alarmsAlarmIdGet(alarm.getId())).withSelfRel().getHref());
		links.setSelf(link);
