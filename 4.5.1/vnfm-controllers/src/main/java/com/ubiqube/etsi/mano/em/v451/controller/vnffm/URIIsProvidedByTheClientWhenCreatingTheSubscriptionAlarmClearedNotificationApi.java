...
@Validated
public interface URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationApi {

	Logger log = LoggerFactory.getLogger(URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationApi.class);
	...
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-AlarmClearedNotification", produces = { "application/json" }, method = RequestMethod.GET)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationGet(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationApi interface so no example is generated");
		}
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "503", description = "503 SERVICE UNAVAILABLE If the API producer encounters an internal overload situation of itself or of a system it relies on, it should respond with this response code, following the provisions in IETF RFC 7231 for the use of the \"Retry-After\" HTTP header and for the alternative to refuse the connection. The \"ProblemDetails\" structure may be omitted. ", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ProblemDetails.class))) })
	@RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-AlarmClearedNotification", produces = { "application/json" }, consumes = { "application/json" }, method = RequestMethod.POST)
	default ResponseEntity<Void> uRIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationPost(@Parameter(in = ParameterIn.HEADER, description = "Version of the API requested to use when responding to this request. ", required = true, schema = @Schema()) @RequestHeader(value = "Version", required = true) final String version, @Parameter(in = ParameterIn.HEADER, description = "The MIME type of the body of the request. Reference: IETF RFC 7231 ", required = true, schema = @Schema()) @RequestHeader(value = "Content-Type", required = true) final String contentType, @Parameter(in = ParameterIn.DEFAULT, description = "Information of the clearance of a VNF alarm.", required = true, schema = @Schema()) @Valid @RequestBody final AlarmClearedNotification body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. ", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) final String authorization) {
		if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
		} else {
			log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionAlarmClearedNotificationApi interface so no example is generated");
		}
