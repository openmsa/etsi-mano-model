...
public class VnfLcmSubscriptions351Sol003Controller implements VnfLcmSubscriptions351Sol003Api, SubscriptionLinkable351Vnfm {
	...
	public VnfLcmSubscriptions351Sol003Controller(final VnfLcmSubscriptionFrontController frontController, final LccnSubscription351Mapping mapper) {
	...
	public ResponseEntity<List<LccnSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return frontController.search(requestParams, nextpageOpaqueMarker, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions351Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsPost(@Valid final LccnSubscriptionRequest body) {
		...
		return frontController.create(req, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions351Sol003Api.class, VnfLcmSubscriptions351Sol003Controller::makeLinks, VnfLcmSubscriptions351Sol003Controller::getSelfLink);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return frontController.findById(subscriptionId, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions351Sol003Controller::makeLinks);
	}
