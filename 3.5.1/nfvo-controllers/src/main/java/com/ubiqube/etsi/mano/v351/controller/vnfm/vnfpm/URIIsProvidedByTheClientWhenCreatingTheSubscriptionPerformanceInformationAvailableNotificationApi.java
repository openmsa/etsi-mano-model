...
import com.ubiqube.etsi.mano.vnfm.v451.model.vnfpm.PerformanceInformationAvailableNotification;
...
@Validated
public interface URIIsProvidedByTheClientWhenCreatingTheSubscriptionPerformanceInformationAvailableNotificationApi {

    Logger log = LoggerFactory.getLogger(URIIsProvidedByTheClientWhenCreatingTheSubscriptionPerformanceInformationAvailableNotificationApi.class);
    ...
    @RequestMapping(value = "/URI_is_provided_by_the_client_when_creating_the_subscription-PerformanceInformationAvailableNotification",
...
", required=true, schema=@Schema()) @Valid @RequestBody PerformanceInformationAvailableNotification body, @Parameter(in = ParameterIn.HEADER, description = "The authorization token for the request. Reference: IETF RFC 7235. " ,schema=@Schema()) @RequestHeader(value="Authorization", required=false) String authorization) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default URIIsProvidedByTheClientWhenCreatingTheSubscriptionPerformanceInformationAvailableNotificationApi interface so no example is generated");
        }
