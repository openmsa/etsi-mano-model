...
import com.ubiqube.etsi.mano.v351.model.vnfm.vnfind.VnfIndicatorValueChangeNotification;

import jakarta.validation.Valid;

@Mapper(componentModel = MappingConstants.ComponentModel.SPRING)
public interface PmJobNotification351Mapping extends DateTimeMapping {
	...
	PerformanceInformationAvailableNotification map(com.ubiqube.etsi.mano.v351.model.vnfm.vnfpm.@Valid PerformanceInformationAvailableNotification body);

	@Mapping(target = "nfvoId", ignore = true)
	@Mapping(target = "subscriptionId", ignore = true)
	@Mapping(target = "performanceValue", ignore = true)
	ThresholdCrossedNotification map(com.ubiqube.etsi.mano.v351.model.vnfm.vnfpm.@Valid ThresholdCrossedNotification body);
