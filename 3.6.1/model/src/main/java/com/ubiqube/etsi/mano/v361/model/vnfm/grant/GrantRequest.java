 ...
 * VNFM. NOTE 4: For the affinity/anti-affinity rules defined in the VNFD and
 * the placement constraints in the GrantVnfLifecycleOperation as defined in
 * this clause, the following applies: Assuming unlimited capacity, the
 * combination of all the aforementioned rules shall be satisfiable by at least
 * one possible placement of the new resources, with the exception that some of
 * the rules with fallbackBestEffort may be unsatisfiable due to the actual
 * placement of existing resources. In this case, rules with fallbackBestEffort
 * are allowed to be violated only in relation to the placement of existing
 * resources. NOTE 5: Passing constraints allows the VNFM or the lifecycle
 * management scripts to influence resource placement decisions by the NFVO to
 * ensure VNF properties such as performance or fault tolerance. NOTE 6: If
 * fallbackBestEffort is present and set to \&quot;true\&quot; in one or more
 * placement constraints and, the NFVO cannot find a resource placement that
 * satisfies all of these due to limited capacity, the NFVO shall process each
 * such affinity/anti-affinity constraint in a best effort manner, in which
 * case, if specified resources cannot be allocated based on the specified
 * placement constraint, the NFVO shall look for an alternate best effort
 * placement for the specified resources to be granted. In the best effort
 * anti-affinity case, the resources are expected to be spread optimally over
 * all available instances of scope (e.g. zones), and in the best effort
 * affinity case, they are expected to be distributed optimally over as few
 * instances of scope as possible. Being able to satisfy a
 * \&quot;best-effort\&quot; constraint in a best effort manner only, as defined
 * above, shall not lead to rejecting the grant.
 */
