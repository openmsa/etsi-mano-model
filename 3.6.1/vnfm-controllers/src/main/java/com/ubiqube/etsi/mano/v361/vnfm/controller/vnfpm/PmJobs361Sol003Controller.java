...
import com.ubiqube.etsi.mano.v361.model.em.vnfpm.PmJobLinks;
...
public class PmJobs361Sol003Controller implements PmJobs361Sol003Api {
	...
	private final PmJob361Mapping mapper;

	public PmJobs361Sol003Controller(final VnfmPmGenericFrontController vnfmPmGenericFrontController, final PmJob361Mapping mapper) {
	...
	public ResponseEntity<String> pmJobsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return vnfmPmGenericFrontController.search(requestParams, x -> mapper.map(x), PmJobs361Sol003Controller::makeLinks, PmJob.class);
	}
	...
	public ResponseEntity<PmJob> pmJobsPmJobIdGet(final String pmJobIdn) {
		return vnfmPmGenericFrontController.findById(UUID.fromString(pmJobIdn), x -> mapper.map(x), PmJobs361Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<PmJob> pmJobsPost(@Valid final CreatePmJobRequest createPmJobRequest) {
		...
		return vnfmPmGenericFrontController.pmJobsPost(req, x -> mapper.map(x), PmJobs361Sol003Controller::makeLinks, PmJobs361Sol003Controller::makeSelf);
	}
