 ...
 */
@RestController
public class VnfLcmSubscriptions361Sol003Controller implements VnfLcmSubscriptions361Sol003Api, SubscriptionLinkable361Vnfm {
	private final VnfLcmSubscriptionFrontController frontController;
	private final LccnSubscription361Mapping mapper;

	public VnfLcmSubscriptions361Sol003Controller(final VnfLcmSubscriptionFrontController frontController, final LccnSubscription361Mapping mapper) {
	...
	public ResponseEntity<List<LccnSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return frontController.search(requestParams, nextpageOpaqueMarker, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions361Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsPost(@Valid final LccnSubscriptionRequest body) {
		...
		return frontController.create(req, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions361Sol003Api.class, VnfLcmSubscriptions361Sol003Controller::makeLinks, VnfLcmSubscriptions361Sol003Controller::getSelfLink);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return frontController.findById(subscriptionId, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions361Sol003Controller::makeLinks);
	}
	...
	public String makeSelfLink(final String id) {
		return linkTo(methodOn(VnfLcmSubscriptions361Sol003Api.class).subscriptionsSubscriptionIdGet(id)).withSelfRel().getHref();
	}
