...
public class Thresholds361Sol003Controller implements Thresholds361Sol003Api {
	...
	public Thresholds361Sol003Controller(final VnfmThresholdFrontController vnfmThresholdFrontController, final Threshold361Mapping mapper) {
	...
	public ResponseEntity<Threshold> thresholdsPost(@Valid final CreateThresholdRequest createThresholdRequest) {
		...
		return vnfmThresholdFrontController.thresholdsCreate(req, x -> mapper.map(x), Thresholds361Sol003Controller::makeLinks, Thresholds361Sol003Controller::getSelfLink);
	}
	...
	public ResponseEntity<Threshold> thresholdsThresholdIdGet(final String thresholdId) {
		return vnfmThresholdFrontController.findById(thresholdId, x -> mapper.map(x), Thresholds361Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<String> thresholdsGet(final MultiValueMap<String, String> requestParams, final String nextpageOpaqueMarker) {
		return vnfmThresholdFrontController.search(requestParams, nextpageOpaqueMarker, x -> mapper.map(x), Thresholds361Sol003Controller::makeLinks, Threshold.class);
	}
