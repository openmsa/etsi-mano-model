...
@Mapper
public interface VimConnectionInfo431Mapping extends ConnectionMapping {
	@Nullable
	default VimConnectionInformation map(final @Nullable VimConnectionInfo vci) {
		if (null == vci) {
			return null;
		}
	...
	@Nullable
	default Map<String, VimConnectionInformation> mapWithNGenericAndList(final @Nullable List<VimConnectionInfo> in) {
		if (null == in) {
			return null;
		}
		return in.stream()
