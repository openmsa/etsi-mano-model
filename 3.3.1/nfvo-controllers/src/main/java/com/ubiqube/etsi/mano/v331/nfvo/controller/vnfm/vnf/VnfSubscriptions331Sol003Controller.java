...
public class VnfSubscriptions331Sol003Controller implements VnfSubscriptions331Sol003Api, SubscriptionLinkable331Nfvo {
	private final VnfSubscriptionSol003FrontController vnfSubscriptionSol03FrontController;
	private final PkgmSubscriptionRequest331Mapping mapper;

	public VnfSubscriptions331Sol003Controller(final VnfSubscriptionSol003FrontController vnfSubscriptionSol03FrontController, final PkgmSubscriptionRequest331Mapping mapper) {
	...
	public ResponseEntity<List<PkgmSubscription>> subscriptionsGet(final String filter) {
		return vnfSubscriptionSol03FrontController.search(filter, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions331Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<PkgmSubscription> subscriptionsPost(final PkgmSubscriptionRequest body) {
		...
		return vnfSubscriptionSol03FrontController.create(req, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions331Sol003Api.class, VnfSubscriptions331Sol003Controller::makeLinks);
	}
	...
	public ResponseEntity<PkgmSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return vnfSubscriptionSol03FrontController.findById(subscriptionId, x -> mapper.map(x, PkgmNotificationsFilter.class), VnfSubscriptions331Sol003Controller::makeLinks);
	}
	...
	public String makeSelfLink(final String id) {
		return linkTo(methodOn(VnfSubscriptions331Sol003Api.class).subscriptionsSubscriptionIdGet(id)).withSelfRel().getHref();
	}
