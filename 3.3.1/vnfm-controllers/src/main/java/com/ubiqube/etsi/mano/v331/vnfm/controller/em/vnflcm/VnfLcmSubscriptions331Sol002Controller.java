 ...
 */
@RestController
public class VnfLcmSubscriptions331Sol002Controller implements VnfLcmSubscriptions331Sol002Api, SubscriptionLinkable331Vnfm {
	private final VnfLcmSubscriptionFrontController frontController;
	private final LccnSubscription331Mapping mapper;

	public VnfLcmSubscriptions331Sol002Controller(final VnfLcmSubscriptionFrontController frontController, final LccnSubscription331Mapping mapper) {
	...
	public ResponseEntity<List<LccnSubscription>> subscriptionsGet(final MultiValueMap<String, String> requestParams, @Valid final String nextpageOpaqueMarker) {
		return frontController.search(requestParams, nextpageOpaqueMarker, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions331Sol002Controller::makeLinks);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsPost(@Valid final LccnSubscriptionRequest body) {
		...
		return frontController.create(req, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions331Sol002Api.class, VnfLcmSubscriptions331Sol002Controller::makeLinks, VnfLcmSubscriptions331Sol002Controller::getSelfLink);
	}
	...
	public ResponseEntity<LccnSubscription> subscriptionsSubscriptionIdGet(final String subscriptionId) {
		return frontController.findById(subscriptionId, x -> mapper.map(x, LifecycleChangeNotificationsFilter.class), VnfLcmSubscriptions331Sol002Controller::makeLinks);
	}
	...
	public String makeSelfLink(final String id) {
		return linkTo(methodOn(VnfLcmSubscriptions331Sol002Api.class).subscriptionsSubscriptionIdGet(id)).withSelfRel().getHref();
	}
